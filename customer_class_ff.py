...
class Customer:
    ...
    def deleteData(self):
        ...
        for i in range(0,len(self)):
            if self[i]['email'] == choice1:
                print('{} 고객님의 정보가 삭제되었습니다.'.format(self[i]['name']))
                del self[i]
                print(self)
                delok = 1
                break
    ...
    def updateData(self):
        while True:
            ...
            if choice2 in ('name','gender','birthyear'):
                self[idx][choice2]=input('수정할 {}을 입력하세요 :'.format(choice2))
                break
            elif choice2 =='exit':
                break
            else:
